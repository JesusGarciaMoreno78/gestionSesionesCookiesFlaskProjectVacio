...
def createsession():
    session['nombre']="Jesús"
    session['apellidos'] = "García Moreno"
...
def createcookie():
    ...
    if request.method == "POST":
        response.set_cookie("nombre", request.form.get("nombre"), max_age=60)
        response.set_cookie("apellidos", request.form.get("apellidos"), max_age=60)
    return response
